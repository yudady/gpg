...
public class PGPFileProcessor {
	...
	public boolean signEncrypt() throws Exception {
		...
		PGPUtils.signEncryptFile(out, this.getInputFileName(), publicKey, secretKey, this.getPassphrase(), this.isAsciiArmored(), this.isIntegrityCheck());
	...
	public boolean verifyDncrypt() throws Exception {
		FileInputStream secretKeyIn = new FileInputStream(secretKeyFileName);
		FileInputStream inputFileStream = new FileInputStream(inputFileName);

		PGPUtils.verifyFile(inputFileStream, secretKeyIn, this.getPassphrase());
