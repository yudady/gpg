...
public class RSAGen {
	...
	public static void main(String args[]) throws Exception {
		...
		//char pass[] = { 'h', 'e', 'l', 'l', 'o' };
		char pass[] = "tommy".toCharArray();
		PGPKeyRingGenerator krgen = generateKeyRingGenerator("alice@example.com", pass);

		// Generate public key ring, dump to file.
		PGPPublicKeyRing pkr = krgen.generatePublicKeyRing();
		ArmoredOutputStream pubout = new ArmoredOutputStream(new BufferedOutputStream(new FileOutputStream("F:/foya/00.work/gpg/src/main/resources/dummy.asc")));
		pkr.encode(pubout);
		pubout.close();

		// Generate private key, dump to file.
		PGPSecretKeyRing skr = krgen.generateSecretKeyRing();
		BufferedOutputStream secout = new BufferedOutputStream(new FileOutputStream("F:/foya/00.work/gpg/src/main/resources/dummy.skr"));
		skr.encode(secout);
		secout.close();
	}
	...
	public final static PGPKeyRingGenerator generateKeyRingGenerator(String id, char[] pass, int s2kcount) throws Exception {
		...
		System.out.println("s2kcount=>" + s2kcount);
		...
		signhashgen.setPreferredHashAlgorithms(false,
				new int[] { HashAlgorithmTags.SHA256, HashAlgorithmTags.SHA1, HashAlgorithmTags.SHA384, HashAlgorithmTags.SHA512, HashAlgorithmTags.SHA224, });
